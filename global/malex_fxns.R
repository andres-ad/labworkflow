...
save_table_as_image_qFALIDSetup <- function(layout_df, filename, path_for_files,input) {
  # Define the grid table
  grid_table <- tableGrob(layout_df,
                          rows = NULL, # Hide row names
                          theme = ttheme_default(
  ...
  header <- textGrob(paste0("Name:", input$qfalid_name_input," ",input$qfalid_surname_input,"\n",
                            "qFALID:", input$qfalid_id_input,"\n",
                            "Date:",input$qfalid_date_input),
                     gp = gpar(fontsize = 16))
  ...
  # Save as a PNG
  png(paste0(path_for_files,"/qFALIDSetup/LayoutImages/",filename), width = width_in_inches * 100, height = height_in_inches * 100, res = 100)
  
  grid.arrange(header, grid_table, ncol = 1, heights = c(1,4))
  
  dev.off()
  shinyalert::shinyalert(title = "Success!", text = paste0("Image saved successfully \n Location: ", path_for_files,"/qFALIDSetup/LayoutImages/",filename), type = "success")
  
  tryCatch({
    drive_folder <- drive_get(as_id("11SdzNwaHOqTYISdzt-ITImcc4NtPzuo3"))
    drive_upload(paste0(path_for_files,"/qFALIDSetup/LayoutImages/",filename), path = drive_folder, name = filename)
    shinyalert::shinyalert(title = "Success!", text = "Image backed up in Google Drive", type = "success")
  }, error = function(e) {
    shinyalert::shinyalert(title = "Warning!", text = "Could not back up image in Google Drive", type = "warning")
  })
